...
) -> list[CurrencyRateSet]:
    return list_rate_sets(limit=limit, book=book)


@router.get("/rates/{rate_date}", response_model=CurrencyRateSet)
def rates_for_date(
    rate_date: str,
    base: str = Query(default="INR"),
    book: str = Query(default="primary"),
) -> CurrencyRateSet:
    record = get_rate_set(rate_date, base, book)
    if record is None:
        raise HTTPException(status_code=404, detail=f"No locked {book} rates for {rate_date}.")
    return record
